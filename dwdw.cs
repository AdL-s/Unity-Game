...
public class CamerffaMovement : MonoBehaviour
{
    [Header("Constants")]
            ...
            if (GetDistanceFromRoof() > 1.12f)
            {
                ...
                if (state == PlayerStates.InAir)
                {
                    if (doubleJump)
                    {
                        airCounter = 0;
                        airTimer = 0;
                        rb.AddForce(((Vector3.up + moveDirection / 2) * jumpForce) / 5, ForceMode.VelocityChange);
                        doubleJump = false;
                        airCounter = 0.015f;
    ...
    public void SwitchState(PlayerStates newState)
    {
        ...
        switch (state)
        {
            ...
            case PlayerStates.InAir:
                ...
                PlayParticlesIn();
                dopadVFX = oldState != PlayerStates.Dashing;
                targetFOV = normalFOV;
                break;
    ...
    */
    ...
    {
        Vector2 force = new Vector2(0, -gravityAcce * AirTimer() * rb.mass * gravityModifier);
